...
use crate::{
    error::{Error, Result},
    transport::{
        message::{MessageHandler, PendingRequests},
...
#[async_trait]
impl Connectable for SseTransport {
...
#[async_trait]
impl RequestSender for SseTransport {
...
#[async_trait]
impl NotifyChannel for SseTransport {
    async fn notify(&self, msg: JsonRpcMessage) -> Result<()> {
        let post_url = &self.get_post_endpoint().await?;

        self.request
            .post(post_url, &serde_json::to_value(&msg)?, None)
            .await
            .map_err(|e| {
                ...
                match maybe_event {
                    Ok(Some(SSE::Event(e))) if e.event_type == "message" => {
                        match serde_json::from_str::<JsonRpcMessage>(&e.data) {
                            ...
                            Err(err) => warn!("Failed to parse SSE message: {err}"),
                        }
                    }
                    Ok(Some(_)) => continue,
                    Ok(None) | Err(_) => {
